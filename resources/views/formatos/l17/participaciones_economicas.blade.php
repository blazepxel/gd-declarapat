...
<div class="form-group{{ $errors->has('ninguno') ? ' has-error' : '' }}">
  <label for="ninguno" class="col-md-4">Ninguna Participación:<code>*</code></label>
  <div class="col-md-8">
    <label>
      <input id="ninguno" type="checkbox" name="ninguno" value="1" tabindex="{{ ++$tabindex }}" @if($datos->ninguno == 1 or old('ninguno') == 1) checked  @endif>
      <code>Palomea en caso de que NO participaciones.</code>
    </label>
  </div>
  <div class="col-md-4">
  </div>
...
<div id="div_tabla">
  <div class="pull-right">
    <a href="{{ url('/declaracion/'.$declaracion->id.'/'.$formato->slug.'/crear') }}" class="btn btn-default btn-sm" tabindex="{{ ++$tabindex }}">
      ...
      Agregar Participación Económica
    </a>
  </div>
</div>
@endsection
...
<table id="tabla" class="table table-hover text-center">
  <caption>PARTICIPACIONES ECONOMICAS</caption>
  <thead>
    <tr>
      <th scope="col">Estatus</th>
      <th scope="col">Persona/Razón Social</th>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <p><strong>¿Realmente deseas borrar esta  participación esta participación económica?</strong></p>
              </div>
