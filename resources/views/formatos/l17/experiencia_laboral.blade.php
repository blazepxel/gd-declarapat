...
@section('nota')
<li>Agrega  tus últimos 3 empleos. Sin contar el actual.</li>
@endsection
...
<div class="form-group{{ $errors->has('ninguno') ? ' has-error' : '' }}">
  <label for="ninguno" class="col-md-4">Ninguna Experiencia:<code>*</code></label>
  <div class="col-md-8">
    <label>
      <input id="ninguno" type="checkbox" name="ninguno" value="1" tabindex="{{ ++$tabindex }}" @if($datos->ninguno == 1 or old('ninguno') == 1) checked  @endif>
      <code>Palomea en caso de que NO tengas experiencia laboral.</code>
    </label>
  </div>
  <div class="col-md-4">
  </div>
...
<table id="tabla" class="table table-hover text-center">
  ...
  <tbody>
    ...
    <tr>
      <td>{{ $fila->sector }}</td>
      <td>@if($fila->institucion){{ $fila->institucion }}@else{{ $fila->razon_social }}@endif</td>
      <td>{{ $fila->puesto_o_cargo }}</td>
      <td>
